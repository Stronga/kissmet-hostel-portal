    ...
    <div className="rounded-xl border border-border bg-white p-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-[13px] text-text-secondary">{allocationBed(allocation)}</p>
        </div>
...
export function RoomPage() {
  ...
  return (
    <>
      <PageHeader title="My Room" description="Your current room and bed from active allocation." />
      ...
      {data.allocation ? (
        <div className="grid gap-5 lg:grid-cols-[1.15fr_0.85fr]">
          <Card>
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div className="min-w-0">
                <p className="inline-flex items-center gap-1 text-sm font-semibold text-text-secondary">
                  ...
                  <InfoHelp label="About allocation">
                    This room and bed come from your active allocation record. Your allocation is to a specific bed inside the room. Room capacity is not used here as a room assignment.
                  </InfoHelp>
                </p>
                <h2 className="mt-1 text-3xl font-bold text-text-primary">{allocationRoom(data.allocation)}</h2>
            ...
            <Link to="/maintenance" className="mt-5 inline-flex min-h-11 items-center rounded-full bg-primary px-4 py-2 text-sm font-semibold text-white">
              Report an issue
            </Link>
          </Card>
          ...
          <div className="space-y-5">
            {booking?.payment_attention_required ? (
              <Card>
                <div role="alert" className="rounded-2xl border border-danger/30 bg-danger/5 p-4 text-sm text-text-primary">
                ...
                <Link to="/payments" className="mt-4 inline-flex min-h-11 items-center rounded-full bg-primary px-4 py-2 text-sm font-semibold text-white">
                  View Payments
                </Link>
              </Card>
            ) : null}
            <Card>
              <h2 className="text-lg font-bold text-text-primary">Booking relationship</h2>
              {booking ? (
                <div className="mt-4 space-y-3">
                  ...
                  <Link to="/booking" className="inline-flex min-h-11 items-center rounded-full border border-border bg-surface px-4 py-2 text-sm font-semibold text-text-primary">
                    View Booking
                  </Link>
                </div>
              ) : (
        ...
        <Card>
          ...
          {booking ? (
            <div className="mt-5 rounded-2xl border border-border bg-[#f7f9fa] p-4 text-sm text-text-secondary">
              <p><span className="font-semibold text-text-primary">Booking:</span> {booking.booking_number} ({statusLabel(booking.status)})</p>
              {pricedRoomLabel(booking) ? (
                <p className="mt-2 inline-flex items-start gap-1">
                  <span>The priced room on your booking is not shown as your assigned room.</span>
                  <InfoHelp label="About room assignment">
                    Your room appears here only after staff create an active bed allocation. The priced room on a booking is not your assigned room.
                  </InfoHelp>
                </p>
              ) : null}
              <Link to="/booking" className="mt-4 inline-flex min-h-11 items-center rounded-full border border-border bg-surface px-4 py-2 text-sm font-semibold text-text-primary">
                View Booking
              </Link>
            </div>
          ) : null}
      ...
      <Card className="mt-5">
        <h2 className="text-lg font-bold text-text-primary">Previous room assignments</h2>
