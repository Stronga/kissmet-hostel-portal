...
import { fetchResidentAllocation, fetchResidentApplications, fetchResidentBookings, fetchResidentDocuments, fetchResidentProfile } from "../api/resident";
...
export function useResidentDashboard() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [documents, applications, bookings, allocation] = await Promise.allSettled([
